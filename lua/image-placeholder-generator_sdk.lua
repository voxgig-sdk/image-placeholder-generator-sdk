...
require("image-placeholder-generator_types")
...
local ImagePlaceholderGeneratorSDK = {}
ImagePlaceholderGeneratorSDK.__index = ImagePlaceholderGeneratorSDK
...
ImagePlaceholderGeneratorSDK._make_feature = _make_feature


function ImagePlaceholderGeneratorSDK.new(options)
  local self = setmetatable({}, ImagePlaceholderGeneratorSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ImagePlaceholderGeneratorSDK:options_map()
...
function ImagePlaceholderGeneratorSDK:get_utility()
  return Utility.copy(self._utility)
end


function ImagePlaceholderGeneratorSDK:get_root_ctx()
  return self._rootctx
end


function ImagePlaceholderGeneratorSDK:prepare(fetchargs)
...
function ImagePlaceholderGeneratorSDK:direct(fetchargs)
...
function ImagePlaceholderGeneratorSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function ImagePlaceholderGeneratorSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "ImagePlaceholderGeneratorSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function ImagePlaceholderGeneratorSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ImagePlaceholderGeneratorSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ImagePlaceholderGeneratorSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ImagePlaceholderGeneratorSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ImagePlaceholderGeneratorSDK.new(sdkopts)
  sdk.mode = "test"
...
return ImagePlaceholderGeneratorSDK
