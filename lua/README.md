# ImagePlaceholderGenerator Lua SDK



The Lua SDK for the ImagePlaceholderGenerator API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:GenerateCustomPlaceholder()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/image-placeholder-generator-sdk/releases)),
...
```
lua/
├── image-placeholder-generator_sdk.lua    -- Main SDK module
...
The main module (`image-placeholder-generator_sdk`) exports the SDK constructor
