...
The Lua SDK for the ImagePlaceholderGenerator API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:GenerateCustomPlaceholder(nil):load({ id = "example_id" }, nil)
...
local result, err = client:ImagePlaceholderGenerator(nil):load(
  { id = "test01" }, nil
)
