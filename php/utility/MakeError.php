...
class ImagePlaceholderGeneratorMakeError
{
    public static function call(?ImagePlaceholderGeneratorContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ImagePlaceholderGeneratorContext([], null);
        }
        $op = $ctx->op ?? new ImagePlaceholderGeneratorOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ImagePlaceholderGeneratorResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ImagePlaceholderGeneratorError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ImagePlaceholderGeneratorError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof ImagePlaceholderGeneratorError) {
