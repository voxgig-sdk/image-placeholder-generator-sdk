# ImagePlaceholderGenerator PHP SDK



The PHP SDK for the ImagePlaceholderGenerator API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->GenerateCustomPlaceholder()` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
try {
    ...
    print_r($generatecustomplaceholder->data_get());
} catch (\Throwable $err) {
...
$client = ImagePlaceholderGeneratorSDK::test([
    "entity" => ["generatesquareplaceholder" => ["test01" => ["id" => "test01"]]],
]);

// Entity ops return the ENTITY (throws on error);
// call data_get() for the mock record.
$generatesquareplaceholder = $client->GenerateSquarePlaceholder()->load(["id" => "test01"]);
print_r($generatesquareplaceholder->data_get());
```
...
```php
$client = ImagePlaceholderGeneratorSDK::test($testopts, $sdkopts);
```
...
| `GenerateCustomPlaceholder` | `($data): GenerateCustomPlaceholderEntity` | Create a GenerateCustomPlaceholder entity instance. |
| `GenerateRectangularPlaceholder` | `($data): GenerateRectangularPlaceholderEntity` | Create a GenerateRectangularPlaceholder entity instance. |
| `GenerateSquarePlaceholder` | `($data): GenerateSquarePlaceholderEntity` | Create a GenerateSquarePlaceholder entity instance. |
...
```
php/
├── imageplaceholdergenerator_sdk.php          -- Main SDK class
...
The main class (`imageplaceholdergenerator_sdk.php`) exports the SDK class
