...
class GenerateSquarePlaceholderEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ImagePlaceholderGeneratorSDK::test(null, null);
        $ent = $testsdk->GenerateSquarePlaceholder(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = generate_square_placeholder_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "generate_square_placeholder." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IMAGEPLACEHOLDERGENERATOR_TEST_GENERATE_SQUARE_PLACEHOLDER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $generate_square_placeholder_ref01_data = null;
        if (count($generate_square_placeholder_ref01_data_raw) > 0) {
        ...
        [$generate_square_placeholder_ref01_data_dt0_loaded, $err] = $generate_square_placeholder_ref01_ent->load($generate_square_placeholder_ref01_match_dt0, null);
        $this->assertNull($err);
...
function generate_square_placeholder_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/generate_square_placeholder/GenerateSquarePlaceholderTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["generate_square_placeholder01", "generate_square_placeholder02", "generate_square_placeholder03"] as $k) {
    ...
    $entid_env_raw = getenv("IMAGEPLACEHOLDERGENERATOR_TEST_GENERATE_SQUARE_PLACEHOLDER_ENTID");
