...
// ImagePlaceholderGenerator SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ImagePlaceholderGeneratorOmni::makeStructRunner(
    ...
    {
        return ImagePlaceholderGeneratorOmni::objmodel(ImagePlaceholderGeneratorOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return ImagePlaceholderGeneratorOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return ImagePlaceholderGeneratorOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ImagePlaceholderGeneratorOmni::structfix($now, Struct::undef()))
                        !== json_encode(ImagePlaceholderGeneratorOmni::structfix($val, Struct::undef()))) {
