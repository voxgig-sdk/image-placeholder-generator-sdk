...
// ImagePlaceholderGenerator SDK feature test
//
...
class FtCtrl extends ImagePlaceholderGeneratorControl
{
...
class FtClient
{
    ...
    private ImagePlaceholderGeneratorUtility $utility;

    public function __construct(ImagePlaceholderGeneratorUtility $utility, array $options)
    {
    ...
    public function get_utility(): ImagePlaceholderGeneratorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ImagePlaceholderGeneratorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ImagePlaceholderGeneratorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ImagePlaceholderGeneratorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ImagePlaceholderGeneratorConfig::make_feature($name);
    ...
    private static function buildUrl(ImagePlaceholderGeneratorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ImagePlaceholderGeneratorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ImagePlaceholderGeneratorResponse($fetched);
        }
    ...
    private function fail(ImagePlaceholderGeneratorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ImagePlaceholderGeneratorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
