...
//    only - a typed ImagePlaceholderGeneratorContext there would make every ctx
...
final class ImagePlaceholderGeneratorOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ImagePlaceholderGeneratorContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new ImagePlaceholderGeneratorSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new ImagePlaceholderGeneratorResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new ImagePlaceholderGeneratorError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new ImagePlaceholderGeneratorResponse($clean['response']);
    ...
    public static function err_from_map($m): ?ImagePlaceholderGeneratorError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ImagePlaceholderGeneratorError(is_string($code) ? $code : '', $msg);
    }
