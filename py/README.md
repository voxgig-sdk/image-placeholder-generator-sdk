...
The Python SDK for the ImagePlaceholderGenerator API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.GenerateCustomPlaceholder(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.ImagePlaceholderGenerator(None).load(
    {"id": "test01"}, None
)
