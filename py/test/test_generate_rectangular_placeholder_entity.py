    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set IMAGE_PLACEHOLDER_GENERATOR_TEST_GENERATE_RECTANGULAR_PLACEHOLDER_ENTID JSON to run live")
        client = setup["client"]
        ...
        generate_rectangular_placeholder_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(generate_rectangular_placeholder_ref01_data_dt0_loaded))
        assert generate_rectangular_placeholder_ref01_data_dt0_load_result is not None
        assert generate_rectangular_placeholder_ref01_data_dt0_load_result["id"] == generate_rectangular_placeholder_ref01_data["id"]
...
def _generate_rectangular_placeholder_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("IMAGE_PLACEHOLDER_GENERATOR_TEST_GENERATE_RECTANGULAR_PLACEHOLDER_ENTID"))
