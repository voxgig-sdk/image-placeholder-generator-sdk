    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set IMAGEPLACEHOLDERGENERATOR_TEST_GENERATE_CUSTOM_PLACEHOLDER_ENTID JSON to run live")
        client = setup["client"]
        ...
        generate_custom_placeholder_ref01_data_dt0_loaded, err = generate_custom_placeholder_ref01_ent.load(generate_custom_placeholder_ref01_match_dt0, None)
        assert err is None
...
def _generate_custom_placeholder_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("IMAGEPLACEHOLDERGENERATOR_TEST_GENERATE_CUSTOM_PLACEHOLDER_ENTID"))
