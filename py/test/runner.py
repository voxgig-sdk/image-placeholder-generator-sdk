# ImagePlaceholderGenerator SDK test runner
...
class ImagePlaceholderGeneratorTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ImagePlaceholderGeneratorTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ImagePlaceholderGeneratorTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ImagePlaceholderGeneratorTestRunner.getenv("IMAGE_PLACEHOLDER_GENERATOR_TEST_LIVE")
        override = ImagePlaceholderGeneratorTestRunner.getenv("IMAGE_PLACEHOLDER_GENERATOR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ImagePlaceholderGeneratorTestRunner.getenv(key)
        ...
        explain = ImagePlaceholderGeneratorTestRunner.getenv("IMAGE_PLACEHOLDER_GENERATOR_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ImagePlaceholderGeneratorTestRunner._test_control is not None:
            return ImagePlaceholderGeneratorTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ImagePlaceholderGeneratorTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ImagePlaceholderGeneratorTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ImagePlaceholderGeneratorTestRunner.load_test_control()
...
def load_env_local():
    ImagePlaceholderGeneratorTestRunner.load_env_local()


def env_override(m):
    return ImagePlaceholderGeneratorTestRunner.env_override(m)


def entity_data(v):
    return ImagePlaceholderGeneratorTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ImagePlaceholderGeneratorTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ImagePlaceholderGeneratorTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ImagePlaceholderGeneratorTestRunner.load_test_control()


def live_client_options():
    return ImagePlaceholderGeneratorTestRunner.live_client_options()


def live_delay_ms():
    return ImagePlaceholderGeneratorTestRunner.live_delay_ms()
