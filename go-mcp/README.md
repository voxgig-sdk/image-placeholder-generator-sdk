# image-placeholder-generator-mcp

[MCP](https://modelcontextprotocol.io) server exposing the ImagePlaceholderGenerator SDK as
two agent tools — `image-placeholder-generator_list` and `image-placeholder-generator_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/image-placeholder-generator-mcp)
make build

# 2. Provide credentials via the environment
export IMAGE_PLACEHOLDER_GENERATOR_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user image-placeholder-generator \
  -- /absolute/path/to/image-placeholder-generator-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./image-placeholder-generator-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// image-placeholder-generator_list: first page of records
{ "entity": "generate_custom_placeholder" }
{ "entity": "generate_custom_placeholder", "query": { } }

// image-placeholder-generator_load: one record by id
{ "entity": "generate_custom_placeholder", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/image-placeholder-generator-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user image-placeholder-generator \
     -- "$PWD"/dist/*/image-placeholder-generator-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `image-placeholder-generator_list` and `image-placeholder-generator_load` tools now appear
   in new sessions. Ask the agent to *"list generate_custom_placeholder using image-placeholder-generator"*
   and it calls `image-placeholder-generator_list` with `{"entity":"generate_custom_placeholder"}`.
...
```sh
export IMAGE_PLACEHOLDER_GENERATOR_APIKEY=sk_live_xxx            # API key
export IMAGE_PLACEHOLDER_GENERATOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `image-placeholder-generator_list` tool
...
### Call the `image-placeholder-generator_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `image-placeholder-generator_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `image-placeholder-generator_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IMAGE_PLACEHOLDER_GENERATOR_APIKEY` | API key sent with every request. |
| `IMAGE_PLACEHOLDER_GENERATOR_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

generate_custom_placeholder | generate_rectangular_placeholder | generate_square_placeholder

### Smoke test via HTTP (raw JSON-RPC)

```sh
./image-placeholder-generator-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"image-placeholder-generator_load","arguments":{"entity":"generate_custom_placeholder","query":{"id":1}}}}'
```
