...
// ImagePlaceholderGenerator SDK retry feature
...
class ImagePlaceholderGeneratorRetryFeature extends ImagePlaceholderGeneratorBaseFeature
{
    ...
    public function init(ImagePlaceholderGeneratorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ImagePlaceholderGeneratorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ImagePlaceholderGeneratorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ImagePlaceholderGeneratorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
