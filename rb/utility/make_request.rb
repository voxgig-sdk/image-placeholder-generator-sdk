...
module ImagePlaceholderGeneratorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ImagePlaceholderGeneratorResponse.new({})
    ...
    if fetch_err
      ...
      response = ImagePlaceholderGeneratorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ImagePlaceholderGeneratorResponse.new(fetched)
    else
