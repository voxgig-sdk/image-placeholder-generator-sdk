...
class GenerateRectangularPlaceholderEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "generate_rectangular_placeholder"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    GenerateRectangularPlaceholderEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ImagePlaceholderGeneratorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GenerateRectangularPlaceholder, Hash] the current GenerateRectangularPlaceholder data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ImagePlaceholderGeneratorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GenerateRectangularPlaceholder fields)
  def match_get
  ...
  #
  # @param reqmatch [GenerateRectangularPlaceholderLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GenerateRectangularPlaceholder.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GenerateRectangularPlaceholder, Hash] the loaded GenerateRectangularPlaceholder; raises ImagePlaceholderGeneratorError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ImagePlaceholderGeneratorHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
