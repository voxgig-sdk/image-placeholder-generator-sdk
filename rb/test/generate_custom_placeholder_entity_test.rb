...
class GenerateCustomPlaceholderEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set IMAGEPLACEHOLDERGENERATOR_TEST_GENERATE_CUSTOM_PLACEHOLDER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    generate_custom_placeholder_ref01_data_dt0_loaded, err = generate_custom_placeholder_ref01_ent.load(generate_custom_placeholder_ref01_match_dt0, nil)
    assert_nil err
...
def generate_custom_placeholder_basic_setup(extra)
  ...
  entid_env_raw = ENV["IMAGEPLACEHOLDERGENERATOR_TEST_GENERATE_CUSTOM_PLACEHOLDER_ENTID"]
