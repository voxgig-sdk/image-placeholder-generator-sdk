...
class GenerateSquarePlaceholderDirectTest < Minitest::Test
  def test_direct_load_generate_square_placeholder
    setup = generate_square_placeholder_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-generate_square_placeholder", setup[:live] ? "live" : "unit")
    if _should_skip
...
def generate_square_placeholder_direct_setup(mockres)
