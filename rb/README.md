...
The Ruby SDK for the ImagePlaceholderGenerator API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
gem "image-placeholder-generator-sdk"
```
...
```ruby
result, err = client.GenerateCustomPlaceholder(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.ImagePlaceholderGenerator(nil).load(
  { "id" => "test01" }, nil
)
