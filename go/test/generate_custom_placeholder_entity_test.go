...
func TestGenerateCustomPlaceholderEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.GenerateCustomPlaceholder(nil)
		if ent == nil {
			t.Fatal("expected non-nil GenerateCustomPlaceholderEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := generate_custom_placeholderBasicSetup(nil)
		...
		for _, _op := range []string{"load"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "generate_custom_placeholder." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		_ = generateCustomPlaceholderRef01Data
...
func generate_custom_placeholderBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "generate_custom_placeholder", "GenerateCustomPlaceholderTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read generate_custom_placeholder test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse generate_custom_placeholder test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("IMAGE_PLACEHOLDER_GENERATOR_TEST_GENERATE_CUSTOM_PLACEHOLDER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IMAGE_PLACEHOLDER_GENERATOR_TEST_GENERATE_CUSTOM_PLACEHOLDER_ENTID"])
