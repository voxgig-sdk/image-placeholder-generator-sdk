...
func TestGenerateRectangularPlaceholderEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.GenerateRectangularPlaceholder(nil)
		if ent == nil {
			t.Fatal("expected non-nil GenerateRectangularPlaceholderEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := generate_rectangular_placeholderBasicSetup(nil)
		...
		for _, _op := range []string{"load"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "generate_rectangular_placeholder." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		_ = generateRectangularPlaceholderRef01Data
...
func generate_rectangular_placeholderBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "generate_rectangular_placeholder", "GenerateRectangularPlaceholderTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read generate_rectangular_placeholder test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse generate_rectangular_placeholder test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("IMAGE_PLACEHOLDER_GENERATOR_TEST_GENERATE_RECTANGULAR_PLACEHOLDER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IMAGE_PLACEHOLDER_GENERATOR_TEST_GENERATE_RECTANGULAR_PLACEHOLDER_ENTID"])
