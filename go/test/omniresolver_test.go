...
import (
	...
	omni "github.com/voxgig-sdk/image-placeholder-generator-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ImagePlaceholderGeneratorError); is {
				out := map[string]any{
					"name":    "ImagePlaceholderGeneratorError",
...
func omniCtx(arg any, client *sdk.ImagePlaceholderGeneratorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ImagePlaceholderGeneratorSDK); is {
			client = live
