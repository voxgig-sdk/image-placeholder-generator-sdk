...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("IMAGEPLACEHOLDERGENERATOR_TEST_LIVE") == "TRUE" ||
		os.Getenv("IMAGEPLACEHOLDERGENERATOR_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("IMAGEPLACEHOLDERGENERATOR_TEST_EXPLAIN"); explain != "" {
