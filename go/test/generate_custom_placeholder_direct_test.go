...
func TestGenerateCustomPlaceholderDirect(t *testing.T) {
	t.Run("direct-load-generate_custom_placeholder", func(t *testing.T) {
		setup := generate_custom_placeholderDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-generate_custom_placeholder", _mode); _shouldSkip {
			if _reason == "" {
...
type generate_custom_placeholderDirectSetupResult struct {
...
func generate_custom_placeholderDirectSetup(mockres any) *generate_custom_placeholderDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &generate_custom_placeholderDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &generate_custom_placeholderDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
