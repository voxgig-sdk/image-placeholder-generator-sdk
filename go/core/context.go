...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*ImagePlaceholderGeneratorSDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *ImagePlaceholderGeneratorError {
	return NewImagePlaceholderGeneratorError(code, msg, ctx)
}
