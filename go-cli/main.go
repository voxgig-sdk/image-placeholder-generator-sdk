// Command ImagePlaceholderGenerator-cli is a CLI + REPL for the ImagePlaceholderGenerator SDK,
...
const prompt = "image-placeholder-generator"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "generate_custom_placeholder generate_rectangular_placeholder generate_square_placeholder"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IMAGE_PLACEHOLDER_GENERATOR_APIKEY (api key), IMAGE_PLACEHOLDER_GENERATOR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IMAGE_PLACEHOLDER_GENERATOR_APIKEY carries the API key and
	// IMAGE_PLACEHOLDER_GENERATOR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IMAGE_PLACEHOLDER_GENERATOR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IMAGE_PLACEHOLDER_GENERATOR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewImagePlaceholderGeneratorSDK(opts)
