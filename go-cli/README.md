# image-placeholder-generator-cli

boru-driven command-line client **and** interactive REPL for the ImagePlaceholderGenerator
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/image-placeholder-generator-cli)
make build

# 2. See usage (words, entities, env vars)
./image-placeholder-generator-cli --help

# 3. Provide credentials once, via the environment
export IMAGE_PLACEHOLDER_GENERATOR_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./image-placeholder-generator-cli load 1 generate_custom_placeholder            # {id:1} shorthand
./image-placeholder-generator-cli load '{id:1}' generate_custom_placeholder       # explicit match map

# 5. Override the API base URL for a single call
IMAGE_PLACEHOLDER_GENERATOR_BASE=https://api.example.com ./image-placeholder-generator-cli load 1 generate_custom_placeholder

# 6. No arguments -> interactive REPL
./image-placeholder-generator-cli
image-placeholder-generator> load 1 generate_custom_placeholder
image-placeholder-generator> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/image-placeholder-generator-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/image-placeholder-generator-cli load 1 generate_custom_placeholder
   ```
...
```sh
./image-placeholder-generator-cli load 1 generate_custom_placeholder          # scalar shorthand for {id:1}
./image-placeholder-generator-cli load '{id:1}' generate_custom_placeholder     # explicit match map
```
...
./image-placeholder-generator-cli load 1 generate_custom_placeholder
```
...
Run with no arguments to open a REPL (prompt `image-placeholder-generator>`). Each line is
evaluated as its own boru expression:

```text
$ ./image-placeholder-generator-cli
image-placeholder-generator> load 1 generate_custom_placeholder
image-placeholder-generator> /help
image-placeholder-generator> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `generate_custom_placeholder`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/image-placeholder-generator-cli`. |
...
(`./image-placeholder-generator-cli <expr>`) and interactively (the REPL), and expressions compose the
