...
`client.GenerateCustomPlaceholder()` — each with a small set of operations (`load`)
...
| `tester(testopts?, sdkopts?)` | `ImagePlaceholderGeneratorSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `ImagePlaceholderGeneratorSDK.test(testopts?, sdkopts?)` | `ImagePlaceholderGeneratorSDK` | Create a test-mode client. |
...
| `client` | `client(): ImagePlaceholderGeneratorSDK` | Return the parent SDK client. |
...
```ts
const generate_square_placeholder = await client.GenerateSquarePlaceholder().load({ id: 1 })
```
...
├── src/
│   ├── ImagePlaceholderGeneratorSDK.ts        # Main SDK class
