...
//     (\`new ImagePlaceholderGeneratorSDK(...)\` / \`ImagePlaceholderGeneratorSDK.test(...)\` become
//     \`ImagePlaceholderGeneratorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ImagePlaceholderGeneratorSDK'
...
const TEST_SEED = {"entity":{"generate_custom_placeholder":{"test01":{"id":"test01"}},"generate_rectangular_placeholder":{"test01":{"id":"test01"}},"generate_square_placeholder":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ImagePlaceholderGeneratorSDK(...)\` and any
// \`ImagePlaceholderGeneratorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ImagePlaceholderGeneratorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ImagePlaceholderGeneratorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
